...
이 문서는 M1의 **합성 입력 정렬 기준선**을 설명한다. 이후 구현한 인덱스 / 후보 필터는
[M2_METHOD](M2_METHOD.md), 점수 선택의 근거는 [SCORING_RATIONALE](SCORING_RATIONALE.md)를
참조한다. 실제 3Di 추출 / 공식 도구 비교 / 생물학적 평가 / Numba는 아직 구현하지 않았다.
