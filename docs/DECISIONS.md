...
## D021 — 사용자가 요청한 GitHub 보존

2026-09-06 사용자가 결과물을 GitHub에 올리도록 요청했다. 이 요청에 한해 초기 AGENTS.md의
원격 push 금지보다 사용자의 최신 지시를 우선한다. 공개 범위는 지정되지 않아 비공개 저장소를
기본으로 하며 소스·테스트·보고서·그림과 기존 실측에서 선별한 작은 근거 파일만 올린다.
큰 구조 자료, upstream 행렬/lookup/실행 파일, wheel, 가상환경은 포함하지 않는다.
외부 자산의 라이선스를 변경하거나 저장소 전체에 새 라이선스를 임의로 부여하지 않는다.
원본 Markdown과 동결된 코드·설정·실측 결과는 보존하고, 링크·설치 안내·보존 기록을 정리한다.
이번 새 환경 검증은 기존 결과의 재검증이며 새로운 held-out benchmark가 아니다.
