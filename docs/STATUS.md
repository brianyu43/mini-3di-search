...
2026-09-05: **M0 / M1 completed**, 전체 v0.1은 **partial**이다.
...
M0/M1을 막는 blocker는 없다. `real_integration_passed=false`이며 synthetic 결과를
생물학적 검증으로 해석하지 않는다. 공식 Foldseek는 미설치다. upstream 고정 버전,
실제 matrix hash, 구조 archive 크기/사용 조건, SCOPe 대응 라벨은 M3 전에 확인한다.
실제 DB 다운로드와 M2 이후 구현은 이번에 진행하지 않았다.

다음 단계는 **M2 인덱스 / single-hit / double-hit / ungapped filter**다.
